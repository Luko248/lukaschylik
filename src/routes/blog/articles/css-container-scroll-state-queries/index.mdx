...
Napadlo Vás niekedy, že budeme pomocou CSS niekedy schopní sledovať scroll stav elementu? Mňa teda nie, a to sa CSS venujem už pekných pár rokov. **CSS Container Scroll State Queries menia hru!**
...
Query `scrollable` testuje, či je možné kontajner scrollovať v danom smere pomocou užívateľom iniciovaného scrollovania. Inými slovami, existuje v danom smere pretekajúci obsah, ku ktorému sa dá scrollovať? To nám umožňuje jednoducho identifikovať či je užívateľ na začiatku, na konci alebo v priebehu scrollovania. Scroll stav je možné identifikovať ako na x-ovej, tak na y-ovej osi.
...
Query `snapped` testuje, či je kontajner "prichytený" (snapped) k scroll snap kontajneru. Táto query sa aktivuje len v kombinácii s CSS vlastnosťou `scroll-snap-type`. Rovnako ako `scrollable`, môžeme identifikovať `snapped` ako na x-ovej, tak na y-ovej osi.
...
Query `stuck` testuje, či je kontajner s hodnotou `position: sticky` "prilepený" (stuck) k okraju svojho rodiča, nezávisle na strane, ku ktorej sa element prilepí.
...
Pre prehliadače, ktoré nepodporujú Container Scroll State Queries, môžete implementovať fallback riešenie pomocou feature query `@supports`:
